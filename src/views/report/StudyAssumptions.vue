  ...
  <div class="study-assumptions-page">
    <div class="packaging-information">
      ...
      <div class="package-infomation-content">
        <div class="package-infomation-content-left">
          <div
            >Are there labeled supplies (Label group) only specific for certain
            countries? Yes or no? If yes, please fill into the table on the
            right.</div
          >
        ...
        <div v-if="formValue.hasLabelGroups" class="label-groups-content">
          <div style="margin-bottom: 8px">
            <el-button type="primary" size="small" @click="addLabelGroups"
              >Add</el-button
            >
            <el-button size="small" @click="deleteLabelGroups"
              >Delete</el-button
            >
          </div>
      ...
      <div class="package-infomation-table">
        <div style="margin-bottom: 8px">
          <el-button type="primary" size="small" @click="addPackageInfomation"
            >Add</el-button
          >
          <el-button size="small" @click="deletePackageInfomation"
            >Delete</el-button
          >
        </div>
        <el-table id="data-table1" :data="formValue.packageInfomation" border>
          <el-table-column
            prop="packageInfomation"
            label="Packaging information"
            align="center"
          >
            <template
              v-for="(column, index) of packageInfomationColumn"
              :key="index"
            >
              <el-table-column
                :prop="column.prop"
                ...
                <template #default="scope">
                  ...
                  >
                    <el-input-number
                      ...
                      v-model="scope.row[column.prop]"
                    />
                    <el-button @click="switchUnit(scope.row)">{{
                      scope.row.unit
                    }}</el-button>
                  </div>
                </template>
    ...
    <div class="productions-depot-shipments">
      ...
      <h3 class="module-title-h3"
        >Past IP releases (For revaluation or initial evaluation with historical
        data)</h3
      >
      <div class="productions-depot-past-IMP-releases-table">
        <el-table id="data-table3" :data="formValue.pastIPReleases" border>
          <el-table-column
            prop="releasedPackagingCampaigns"
            label="Already released packaging campaigns (past IMP releases)"
            align="center"
          >
            <template
              v-for="(column, index) of pastIPReleasesColumn"
              :key="index"
            >
              <el-table-column
                :prop="column.prop"
      ...
      <h3 class="module-title-h3"
        >Confirmed Future IP releases ( can leave as empty in case no future
        release plan)</h3
      >
      <div class="productions-depot-future-IMP-releases-table">
        <el-table id="data-table4" :data="formValue.futureIPReleases" border>
          <el-table-column
            prop="futureImpReleases"
            label="Future IMP releases"
            align="center"
          >
            <template
              v-for="(column, index) of futureIPReleasesColumn"
              :key="index"
            >
              <el-table-column
                :prop="column.prop"
      ...
      <div class="preferred-release">
        <div
          >Preferred release frequency per kit type (e.g. max every x
          months)</div
        >
...
import {
  packageInfomationColumn,
  PackageInfomationRowType,
  depotNetworksColumn,
  pastIPReleasesColumn,
  futureIPReleasesColumn
} from './StudyAssumptions';
